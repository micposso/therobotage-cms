        ...
        {!!Object.keys(errors).length && (
          <ul>
            {Object.entries(errors).map(([name, error]) => (
              <li key={name}><a href={`#partner-${name}`}>{error}</a></li>
            ))}
          </ul>
        )}
      </div>
      {status !== 'success' && (
        <>
          <fieldset disabled={status === 'loading'} className={styles.formGrid}>
            <legend className={styles.srOnly}>Robot Literacy partner inquiry details</legend>
              ...
              return (
                <div className={field.type === 'textarea' ? styles.fullField : styles.field} key={field.name}>
                  ...
                  {field.options ? (
                    ...
                    <textarea {...common} rows={6} maxLength={3000} placeholder={field.placeholder} />
                  ) : (
          ...
          <fieldset id="partner-contributions" tabIndex={-1} disabled={status === 'loading'} className={local.interestGroup} aria-describedby={errors.contributions ? 'partner-contributions-error' : undefined}>
            <legend>How could your organization contribute? <span className={styles.consent}>(optional)</span></legend>
            ...
            {errors.contributions && <p id="partner-contributions-error" className={styles.fieldError}>{errors.contributions}</p>}
          </fieldset>
          <label className={checks.checkboxField}>
            ...
            />
            I agree to be contacted by The Robot Age regarding Robot Literacy partnerships.
          </label>
          {errors.consent && <p id="partner-consent-error" className={styles.fieldError}>{errors.consent}</p>}
          <div className={styles.actions}>
            <button className={styles.button} type="submit" disabled={status === 'loading'}>
              {status === 'loading' ? 'Sending inquiry...' : 'Start a Partnership Conversation'}
            </button>
          </div>
        </>
      )}
