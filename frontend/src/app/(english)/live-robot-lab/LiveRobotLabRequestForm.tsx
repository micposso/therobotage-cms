        ...
        {!!Object.keys(errors).length && (
          <ul>
            {labFields
              ...
              .map((field) => (
                <li key={field.name}>
                  <a href={resolveHref(`#lab-${field.name}`)}>
                    {t(field.label)}: {t(errors[field.name] ?? '')}
                  </a>
                </li>
              ))}
          </ul>
      ...
      {status !== 'success' && (
        <>
          <fieldset disabled={status === 'loading'} className={styles.formGrid}>
            <legend className={styles.srOnly}>{t("Live Robot Lab request details")}</legend>
            {labFields.map((field) => {
              ...
              return (
                ...
                >
                  <label htmlFor={id}>
                    {t(field.label)}
                    {t('required' in field && field.required
                      ? ' *'
                      : ' (optional)')}
                  </label>
                  {field.options ? (
          ...
          <p className={styles.consent}>{t("By submitting this form, you agree that The Robot Age may contact you about Live Robot Lab.")}</p>
          <button
            className={styles.button}
            type="submit"
            disabled={status === 'loading'}
          >
            {t(status === 'loading' ? 'Sending request…' : 'Request Information')}
          </button>
        </>
      )}
    </form>
