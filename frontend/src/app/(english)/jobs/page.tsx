...
export const metadata = {
  ...
  alternates: { canonical: '/jobs', languages: translationAlternates('/jobs') },
  openGraph: {
    title: TITLE,
    description: DESCRIPTION,
    url: '/jobs',
...
export default async function JobsPage({ params }: { params: Promise<{ locale?: Locale }> }) {
  ...
  const { jobs: sourceJobs, failed } = await loadJobs()
  const jobs = sourceJobs.map(job => localize(job, `/jobs/${job.slug}`, locale))
  ...
  return (
    <>
      ...
      <PageHero
        eyebrow={t("Jobs")}
        title={t("Product and design roles worth reading about.")}
        subtitle={t("Product, design, UX research, and marketing roles at US robotics companies — not an engineering board. Every listing is read and categorized before it goes up.")}
      ...
      <section className={styles.section}>
        <div className="container-fluid">
          ...
          {failed ? (
            <p className={styles.notice}>{t("The job board is temporarily unavailable. Please try again shortly.")}</p>
          ) : (
            // useSearchParams inside a prerendered route requires this boundary.
            // Without it, next build fails on this route.
            <Suspense fallback={<p className={styles.notice}>{t("Loading roles...")}</p>}>
              <JobBoardExplorer basePath={contentHref('/jobs', locale)} jobs={jobs} facets={facets} />
            </Suspense>
          )}
        </div>
      </section>
