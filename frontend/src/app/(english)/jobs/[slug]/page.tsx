...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; locale?: Locale }> }) {
  ...
  const { job: sourceJob, expired } = resolved
  const job = localize(sourceJob, `/jobs/${slug}`, locale)
  const title = `${job.title} — ${job.companyName}`

  return {
    title: `${title} — The Robot Age`,
    description: job.summary,
    alternates: { canonical: `/jobs/${slug}`, languages: translationAlternates(`/jobs/${slug}`) },
...
export default async function JobDetailPage({ params }: { params: Promise<{ slug: string; locale?: Locale }> }) {
  const { slug, locale = 'en' } = await params
  ...
  let related: JobCard[] = []
  try {
    related = getRelatedJobs(job, await getLiveJobs()).map(item => localize(item, `/jobs/${item.slug}`, locale))
  } catch {
    related = []
  }
        ...
        <div className="container-fluid">
          <div className={styles.layout}>
            <div className={styles.body}>
              {expired && (
                <p className={styles.closed}>{t("This role has closed. It is kept here so existing links still resolve. Open roles are on the")}<Link href="/jobs">{t("job board")}</Link>.
                </p>
              )}
              ...
              {job.companyBlurb && (
                <div className={styles.companyBlock}>
                  <p className={styles.eyebrow}>{t("About ")}{job.companyName}</p>
            ...
            <aside className={styles.sidebar}>
              ...
              <p className={styles.applyNote}>{t("Applications are handled by")}{job.companyName}{t(". The Robot Age does not collect applications or take a fee.")}</p>
