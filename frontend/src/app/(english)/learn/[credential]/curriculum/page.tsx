...
export async function generateMetadata({ params }: { params: Promise<{ credential: string; locale?: Locale }> }) {
  ...
  if (!cert) return {}
  return pageMetadata({
    title: `${cert.abbr} ${uiText('Curriculum', locale)} — The Robot Age`,
    description: uiText('Full module-by-module curriculum for the {credential} credential. See what you will cover, how the programme is structured, and what resources support your learning.', locale).replace('{credential}', uiText(cert.name, locale)),
   }, `/learn/${credential}/curriculum`, locale)
}
...
export default async function CurriculumPage({ params }: { params: Promise<{ credential: string; locale?: Locale }> }) {
  const { credential, locale = 'en' } = await params
      ...
      <main className={styles.page}>
        <div className="container-fluid">
          ...
          <header className={styles.header}>
            <p className={styles.label}>{t(cert.abbr)}{t(" · Curriculum")}</p>
            <h1 className={styles.title}>{t("Six weeks. Six dimensions. One framework.")}</h1>
            <p className={styles.subhead}>{t("The REP programme walks you through the Robot Experience Design framework dimension by dimension — from how robots signal intent to how they recover from failure. Every week builds on the last. Every deliverable is scoped to a real robot.")}</p>
            <div className={styles.actions}>
              <a href="https://learn.therobotage.com" className={styles.cta}>{t("Enroll in the Founding Cohort →")}</a>
              <a href={resolveHref(`/learn/${credential}`)} className={styles.secondary}>{t("← Back to ")}{t(cert.abbr)}{t(" Overview")}</a>
            </div>
            <div className={styles.pills}>
              {['6 Weeks', '~15 Hours Total', '1 Live Zoom + Async', 'Reachy Mini Platform', 'REP Credential on Completion'].map((pill) => (
                <span key={pill} className={styles.pill}>{t(pill)}</span>
              ))}
            </div>
          </header>
          ...
          <section className={styles.section}>
            <p className={styles.eyebrow}>{t("Who this is for")}</p>
            <p className={styles.audienceIntro}>{t("The REP is designed for professionals who work in, around, or alongside robotic systems — but who are not engineers. If your job involves making decisions about how robots show up in human environments, this curriculum was built for you.")}</p>
            <div className={styles.audienceGrid}>
              <div className={styles.audienceCard}>
                <h3 className={styles.audienceCardTitle}>{t("Product Designers & UX Strategists")}</h3>
                <p className={styles.audienceCardBody}>{t("You design the interfaces, environments, and interactions where robots meet people. RXD gives you a structured evaluation method for those moments.")}</p>
              </div>
              <div className={styles.audienceCard}>
                <h3 className={styles.audienceCardTitle}>{t("Operations & Deployment Leads")}</h3>
                <p className={styles.audienceCardBody}>{t("You manage the rollout. You see what breaks. RXD gives you the language to diagnose experience failures and design recovery protocols.")}</p>
              </div>
              <div className={styles.audienceCard}>
                <h3 className={styles.audienceCardTitle}>{t("Business Leaders & Strategists")}</h3>
                <p className={styles.audienceCardBody}>{t("You approve the budget. RXD gives you a scoring instrument to evaluate deployment readiness before signing off — not after.")}</p>
              </div>
            </div>
          </section>
          ...
          {/* ── Programme structure ──────────────────────────────────────────── */}
          <section className={styles.section}>
            <p className={styles.eyebrow}>{t("Programme structure")}</p>
            <p className={styles.structureIntro}>{t("Week 1 is live on Zoom. Weeks 2 through 5 are self-paced with one scoped deliverable each. Week 6 is the capstone — a full Robot Readiness Audit scored against all six RXD dimensions. You submit it, get cohort feedback, and earn the REP credential.")}</p>
            <div className={styles.scheduleTable}>
              <div className={styles.scheduleHeader}>
                <span className={styles.scheduleCol}>{t("Week")}</span>
                <span className={styles.scheduleCol}>{t("Format")}</span>
                <span className={styles.scheduleColWide}>{t("RXD Dimension(s)")}</span>
              </div>
              {[
                ...
                <div key={row.week} className={styles.scheduleRow}>
                  <span className={styles.scheduleCol}>{t(row.week)}</span>
                  <span className={styles.scheduleCol}>{t(row.format)}</span>
                  <span className={styles.scheduleColWide}>{t(row.dimensions)}</span>
                </div>
              ))}
            </div>
          </section>
              ...
              {weeks.map((week, i) => (
                <details key={week.number} className={styles.weekDetails} open={i === 0}>
                  <summary className={styles.weekSummary}>
                    <span className={styles.weekNum}>{t(week.number)}</span>
                    <span className={styles.weekName}>{t(week.title)}</span>
                    <span className={styles.weekTag}>{t(week.format)}</span>
                  </summary>
                  <div className={styles.weekBody}>
                    <p className={styles.weekDimLabel}>{t(week.dimensions)}</p>
                    <p className={styles.weekDesc}>{t(week.description)}</p>
                    <p className={styles.subLabel}>{t("Topics covered")}</p>
                    ...
                    <p className={styles.subLabel}>{t("Deliverable")}</p>
                    <p className={styles.weekDeliv}>{t(week.deliverable)}</p>
                    {week.outcomes.length > 0 && (
                      <>
                        <p className={styles.subLabel}>{t("Learning outcomes")}</p>
                        <ul className={styles.topicList}>
          ...
          <section className={styles.section}>
            <p className={styles.eyebrow}>{t("What you will walk away with")}</p>
          ...
          {/* ── Your platform ─────────────────────────────────────────────────── */}
          <section className={styles.section}>
            <p className={styles.eyebrow}>{t("Your platform")}</p>
            <h2 className={styles.sectionHeadline}>{t("Built around Reachy Mini")}</h2>
            <p className={styles.sectionBody}>{t("Every module uses Reachy Mini — an open-source social humanoid robot developed by Pollen Robotics in partnership with Hugging Face. Reachy Mini is expressive, camera-equipped, voice-capable, and designed for exactly the kind of human-robot interaction this programme evaluates. You do not need to own one. TRA operates the hardware; you analyse the experience.")}</p>
            <div className={styles.pills}>
          ...
          {/* ── The framework ─────────────────────────────────────────────────── */}
          <section className={styles.section}>
            <p className={styles.eyebrow}>{t("The framework behind the curriculum")}</p>
            <h2 className={styles.sectionHeadline}>{t("Robot Experience Design (RXD)")}</h2>
            <p className={styles.sectionBody}>{t("RXD is the original research framework developed by The Robot Age. It defines six dimensions for evaluating how humans experience robots — not how robots are engineered, but how they show up in human environments. The REP curriculum teaches you to apply it. The Robot Experience Score (RES) is the scored instrument. The Robot Readiness Audit (RRA) is the professional application.")}</p>
          ...
          <section className={styles.ctaSection}>
            <p className={styles.eyebrow}>{t("Founding Cohort — Limited to 10 Seats")}</p>
            <h2 className={styles.ctaHeadline}>{t("Be one of the first Robotics Experience Practitioners.")}</h2>
            <p className={styles.ctaBody}>{t("The founding cohort gets direct access to the course creator, input on the curriculum as it evolves, and a permanent founding member designation on their credential. This rate will not be repeated.")}</p>
            <div className={styles.pricingTable}>
              <div className={styles.pricingRow}>
                <span className={styles.pricingLabel}>{t("Founding Cohort Rate")}</span>
                <span className={styles.pricingValueAccent}>
                  $199 <span className={styles.pricingMeta}>{t("or 2 × $99")}</span>
                </span>
              </div>
              <div className={styles.pricingRow}>
                <span className={styles.pricingLabel}>{t("General Rate (future cohorts)")}</span>
                <span className={styles.pricingValue}>$399</span>
              </div>
            </div>
            <a href="https://learn.therobotage.com" className={styles.cta}>{t("Enroll Now →")}</a>
            <p className={styles.ctaSupport}>{t("Payment handled securely via Lemon Squeezy. Credential issued via Credential.net upon completion.")}</p>
          </section>

          {/* ── FAQ ──────────────────────────────────────────────────────────── */}
          <section className={styles.section}>
            <p className={styles.eyebrow}>{t("Common questions")}</p>
            <div className={styles.faqList}>
              {faqs.map((item, i) => (
                <details key={i} className={styles.faqDetails}>
                  <summary className={styles.faqSummary}>
                    <span className={styles.faqQuestion}>{t(item.q)}</span>
                  </summary>
          ...
          <div className={styles.back}>
            <a href={resolveHref(`/learn/${credential}`)} className={styles.backLink}>{t("← Back to ")}{t(cert.name)}
            </a>
          </div>
