...
const WAITLIST_FILE = '/tmp/rxd-workshop-waitlist.json'
const MAX_SEATS     = 15

let _emailCache: Set<string> | null = null
let _waitlistCache: Set<string> | null = null
...
function getWaitlist(): Set<string> {
  if (_waitlistCache) return _waitlistCache
  try {
    const raw = fs.readFileSync(WAITLIST_FILE, 'utf-8')
    _waitlistCache = new Set(JSON.parse(raw) as string[])
  } catch {
    _waitlistCache = new Set()
  }
  return _waitlistCache
}

function persistWaitlist(email: string): void {
  const waitlist = getWaitlist()
  waitlist.add(email)
  try {
    fs.writeFileSync(WAITLIST_FILE, JSON.stringify([...waitlist]))
  } catch {
    // in-memory cache still prevents duplicates within this instance
  }
}
...
export async function sendWorkshopEmail(
  prevState: { success: boolean; waitlisted?: boolean; error?: string } | null,
  formData: FormData
  ...
  const isFull = getRegistry().size >= MAX_SEATS
  ...
  try {
    ...
    if (isFull) {
      const [{ error: confirmError }, { error: adminError }] = await Promise.all([
        ...
        // Waitlist notice to registrant
        resend.emails.send({
          from: FROM_ADDRESS,
          to: email,
          subject: "You're on the waitlist — RXD Free Workshop",
          html: workshopWaitlistHtml(safeFirst),
        }),
        ...
        resend.emails.send({
          from: FROM_ADDRESS,
          to: ADMIN_ADDRESS,
          subject: `Workshop waitlist — ${safeFull}`,
          html: emailHtml(`
      ...
      persistWaitlist(normalizedEmail)
