...
That is a robotics architecture, but it also exposes a product architecture. Instead of designing around robot hardware as the fixed center of the experience, teams can begin designing around robot behavior: what the system understands, how it plans, how it signals state, how it collaborates, and how it recovers when the physical world does not cooperate.

## The new unit of design is the task

Traditional software teams design user flows: sign up, search, compare, purchase, share. Robotic systems require something different because the fundamental unit is no longer the screen. It is the task. Every deployment starts by asking what the person is trying to accomplish, how the robot should interpret that request, what constraints apply, how much autonomy is acceptable, and what should count as success.

DeepMind says Gemini Robotics 2 can plan multi-step activities over several minutes while tracking progress and determining when a task has been completed. That shifts the design challenge away from interface polish and toward task boundaries, permissions, defaults, escalation rules, completion criteria, and recovery behavior. This is familiar territory for product managers, even if the medium has changed from software workflows to physical work.
...
Software products expose progress through loading indicators, confirmation screens, draft states, notifications, and activity timelines. Robots need an equivalent vocabulary because people sharing a workspace with a robot need to know whether it is planning, executing, waiting, retrying, blocked, requesting help, or finished. The most useful robot may not be the fastest one. It may be the one whose internal state is easiest for humans to understand. Designing those states is product work.
...
Perhaps the biggest implication of Gemini Robotics 2 is that robots are no longer operating alone. The emerging architecture looks less like a single autonomous machine and more like an orchestrated system moving across humans, reasoning agents, multiple robots, software tools, enterprise APIs, and other human decision-makers.
...
Google does not present Gemini Robotics 2 as a product framework, but its architecture suggests one. Robotics product teams can organize around intent, context, planning, action, progress, collaboration, safety, and recovery. Intent defines what the human wants to accomplish. Context captures the environment, constraints, and available information. Planning describes how the system decomposes the task. Action covers execution through robot capabilities and connected tools. Progress makes state, milestones, and completion visible. Collaboration coordinates humans, software, and multiple robots. Safety defines when the system refuses, pauses, asks, or escalates. Recovery explains failure, restores confidence, and returns control.

This is not Google's framework. It is a product lens that emerges from the capabilities Google has exposed, and it gives non-engineering teams a practical way to reason about robotic experiences before those experiences harden into hardware behavior.
